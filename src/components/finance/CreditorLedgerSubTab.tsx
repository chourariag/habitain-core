...
export function CreditorLedgerSubTab({ canUpload }: { canUpload: boolean }) {
  const [entries, setEntries] = useState<LedgerEntry[]>([]);
  const [search, setSearch] = useState("");

  const fetchData = async () => {
    const { data } = await supabase.from("creditor_ledger_entries").select("*").order("overdue_days", { ascending: false });
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Clear existing
      await supabase.from("creditor_ledger_entries").delete().neq("id", "00000000-0000-0000-0000-000000000000");
      ...
      for (let i = 0; i < toInsert.length; i += 50) {
        const { error } = await supabase.from("creditor_ledger_entries").insert(toInsert.slice(i, i + 50));
        if (error) { toast.error(error.message); return; }
        imported += toInsert.slice(i, i + 50).length;
      }
      toast.success(`${imported} creditor entries imported`);
      fetchData();
    } catch (err: any) { toast.error(err.message); }
    e.target.value = "";
  };

  const totalPayables = entries.reduce((s, e) => s + e.amount, 0);
  ...
  const overdueVendors = new Set(entries.filter(e => e.status === "overdue").map(e => e.party_name)).size;
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg px-4 py-3 text-sm" style={{ backgroundColor: "#FFF3CD", color: "#664D03" }}>
        <strong>Export from Tally:</strong> Gateway → Display → More Reports → Statements of Accounts → Outstanding → Payables → Set Date → Export Excel. Upload the file here.
      </div>

      {canUpload && (
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" onClick={() => { const t = TEMPLATES.creditorLedger; downloadXlsxTemplate(t.filename, t.sheet, t.headers, t.sample); }} className="gap-1.5" style={{ borderColor: "#006039", color: "#006039" }}>
          ...
          <label>
            ...
            <Button variant="default" asChild style={{ backgroundColor: "#006039" }}>
              <span className="cursor-pointer flex items-center gap-2"><Upload className="h-4 w-4" /> Upload Creditor Ledger</span>
            </Button>
          </label>
        </div>
      )}
      ...
      {entries.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card><CardContent className="pt-3 pb-3">
            <p className="text-[10px] font-display uppercase" style={{ color: "#666" }}>Total Payables</p>
            <p className="text-lg font-mono font-bold" style={{ color: totalPayables > 0 ? "#D4860A" : "#006039" }}>₹{totalPayables.toLocaleString("en-IN")}</p>
          </CardContent></Card>
          <Card><CardContent className="pt-3 pb-3">
            <p className="text-[10px] font-display uppercase" style={{ color: "#666" }}>Overdue Amount</p>
          ...
          <Card><CardContent className="pt-3 pb-3">
            <p className="text-[10px] font-display uppercase" style={{ color: "#666" }}>Vendors Overdue</p>
            <p className="text-lg font-mono font-bold" style={{ color: overdueVendors > 0 ? "#F40009" : "#006039" }}>{overdueVendors}</p>
          </CardContent></Card>
        </div>
      )}
      ...
      <Card>
        <CardContent className="pt-4 overflow-x-auto">
          ...
          {entries.length === 0 && <p className="text-center text-xs py-8" style={{ color: "#999" }}>No creditor data. Upload a Tally export to begin.</p>}
        </CardContent>
      </Card>
    </div>
  );
