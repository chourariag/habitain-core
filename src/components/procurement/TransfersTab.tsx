...
import { Loader2, Truck, Check, Package, Wrench, Plus, ChevronDown } from "lucide-react";
...
// Fix #1: in_progress label corrected from "Pending" to "In Progress"
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
  pending:     { label: "Pending",     color: "#666666", bg: "#E0E0E0" },
  in_progress: { label: "In Progress", color: "#D4860A", bg: "#FFF8E8" },
...
export function TransfersTab() {
  ...
  const fetchTransfers = useCallback(async () => {
    ...
    (profiles ?? []).forEach((p: any) => {
      // Fix #2: use display_name instead of full_name
  ...
  const filtered = useMemo(() => {
    return transfers.filter((t) => {
      ...
      if (filterStatus !== "all") {
        const mappedLabel = STATUS_MAP[t.status]?.label?.toLowerCase() ?? "";
        if (filterStatus === "pending" && mappedLabel !== "pending") return false;
        // Fix #1 follow-up: "in_progress" filter now correctly matches "In Progress" label
        if (filterStatus === "in_progress" && mappedLabel !== "in progress") return false;
        if (filterStatus === "in_transit" && mappedLabel !== "in transit") return false;
        if (filterStatus === "delivered" && mappedLabel !== "delivered") return false;
      }
      if (filterFrom) {
        const from = parseISO(filterFrom);
        if (parseISO(t.created_at) < from) return false;
      }
      if (filterTo) {
        const to = parseISO(filterTo);
        if (parseISO(t.created_at) > to) return false;
      }
      return true;
    });
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
        ...
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-[150px] shrink-0 text-sm">
