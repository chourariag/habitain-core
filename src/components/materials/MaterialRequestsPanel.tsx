...
export function MaterialRequestsPanel({ projectId }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [reqRes, roleRes] = await Promise.all([
      supabase.from("material_requests" as any).select("*").eq("is_archived", false).eq("project_id", projectId).order("created_at", { ascending: false }),
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-foreground">Material Requests</h3>
        {canRequest && <Button size="sm" onClick={() => setAddOpen(true)}><Plus className="h-4 w-4 mr-1" /> New Request</Button>}
      </div>

      {requests.length === 0 ? (
        <div className="bg-card rounded-lg p-8 text-center">
          <PackagePlus className="h-8 w-8 mx-auto text-muted-foreground mb-2" />
          <p className="text-sm text-muted-foreground">No material requests for this project.</p>
        </div>
      ) : (
