      ...
      <table className="w-full text-sm">
        <thead>
          <tr style={{ backgroundColor: "#F7F7F7" }}>
            {["Month", "Gross ₹", "Deductions ₹", "Net Pay ₹", "Action"].map(h => (
        ...
        <tbody>
          {slips.length === 0 ? (
            <tr><td colSpan={5} className="px-3 py-12 text-center text-sm" style={{ color: "#999" }}>
              <FileText className="h-8 w-8 mx-auto mb-2 opacity-40" />
              No payslips uploaded yet. Mary will upload them monthly.
            </td></tr>
          ) : slips.map(s => (
