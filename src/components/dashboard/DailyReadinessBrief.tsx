...
async function buildSalesExecBrief(uid: string): Promise<BriefLine[]> {
  ...
  const [todayRes, followupRes, stagnantRes] = await Promise.all([
    (supabase.from("sales_deals") as any).select("id").in("assigned_to", uid ? [uid] : []).eq("next_followup_date", today).eq("is_archived", false),
    (supabase.from("sales_deals") as any).select("client_name").in("assigned_to", uid ? [uid] : []).eq("next_followup_date", today).eq("is_archived", false),
    (supabase.from("sales_deals") as any).select("id").in("assigned_to", uid ? [uid] : []).not("stage", "in", "(Won,Lost)").lt("updated_at", stagnantCutoff).eq("is_archived", false),
  ]);
  const followupNames = (followupRes.data || []).slice(0, 3).map((d: any) => d.client_name).join(", ") || "none";
  return [
