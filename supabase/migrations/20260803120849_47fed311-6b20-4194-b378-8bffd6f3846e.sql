  ...
  SELECT _object_name LIKE 'design-stages/%'
    AND (
      public.user_has_any_role(_uid, ARRAY[
        ...
        'head_operations','head_of_projects','director'
      ]::app_role[])
      OR EXISTS (
        SELECT 1
...
CREATE POLICY "Stage owners upload design stage files"
ON storage.objects FOR INSERT TO authenticated
WITH CHECK (bucket_id = 'design-files' AND public.can_upload_stage_files(auth.uid(), name));

CREATE POLICY "Authenticated read design stage files"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'design-files' AND name LIKE 'design-stages/%');

CREATE POLICY "Uploader or admin delete design stage files"
ON storage.objects FOR DELETE TO authenticated
USING (
  bucket_id = 'design-files'
  AND name LIKE 'design-stages/%'
  AND (owner = auth.uid() OR public.user_has_any_role(auth.uid(), ARRAY['super_admin','managing_director']::app_role[]))
);
