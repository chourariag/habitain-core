...
BEGIN
  ...
  SELECT p.id, p.auth_user_id, p.display_name, p.email, p.role, p.language,
         p.reporting_manager_id, p.is_active, p.login_type::text, p.is_archived,
         p.created_at, p.updated_at, p.avatar_url, ppi.home_base,
         p.onboarding_completed, p.onboarding_completed_at, p.onboarding_quiz_scores,
         p.department, p.secondary_manager_id
  FROM public.profiles p
  LEFT JOIN public.profile_private_info ppi ON ppi.profile_id = p.id
