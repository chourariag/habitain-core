  ...
  LOOP
    ...
    IF pid IS NOT NULL THEN
      IF EXISTS (SELECT 1 FROM public.projects WHERE id = pid) THEN
        RETURN public.user_can_access_project(_uid, pid) OR is_staff;
      END IF;

      SELECT m.project_id INTO mid FROM public.modules m WHERE m.id = pid;
      IF mid IS NOT NULL THEN
        RETURN public.user_can_access_project(_uid, mid) OR is_staff;
      END IF;
    END IF;
  END LOOP;

  -- No resolvable project scope: allow active internal staff, otherwise deny
